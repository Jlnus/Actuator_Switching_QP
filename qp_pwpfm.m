...
% 추력기 모멘텀 덤핑 수행
% loop : PD -> quadprog -> pwpfm -> rk4(dynamics)
...
% PWPFM 변수
filtered_u = [0 ;0];
PF.MPW = 0.025; % 최소 펄스 수
PF.tau_m = 0.1; % time constant
PF.U_on = 0.15;
PF.U_off = 0.05;
PF.K_m = 10;
...
for i = 1:N-1
    ...
    % ===== PWPFM =====
    filtered_u = filtered_u + dt/PF.tau_m * (PF.K_m * z(5:6) - filtered_u); % 1st filter
    u_pwpfm = PWPFM_thr(filtered_u,PF,t(i));

    tau(i,1:4) = z(1:4)'; % rw
    tau(i,5:6) = u_pwpfm; % thr
    s1(i,:) = z(7:9)';
    s2(i,:) = z(10:12)';
